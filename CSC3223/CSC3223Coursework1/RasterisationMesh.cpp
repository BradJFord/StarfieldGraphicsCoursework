...
using namespace CSC3223;
	...
	RasterisationMesh* RasterisationMesh::CreateLineFromPoints(const std::vector <Vector3>& inVerts,
	...
	void RasterisationMesh::RasterBresenhamLine(
		const Vector3& v0, const Vector3& v1,
		const Vector4& c0, const Vector4& c1) {
		Vector3 dir = v1 - v0;
		...
		Vector4 colourDelta = colourDiff / scanRange;// change per pixel
	...
	RasterisationMesh* RasterisationMesh::CreateTriangleFromPoints(const std::vector < Vector3 >& inVerts,
		const std::vector<Vector4>& colours, int type) {
	...
	void RasterisationMesh::RasterLineEquationTriangle(
		...
		const Vector4& c0, const Vector4& c1, const Vector4& c2){
			...
			Vector3 linev0v1 = Vector3::Cross(Vector3(p0.x, p0.y, 1),
			Vector3(p1.x, p1.y, 1));
			Vector3 linev1v2 = Vector3::Cross(Vector3(p1.x, p1.y, 1),
			Vector3(p2.x, p2.y, 1));
			Vector3 linev2v0 = Vector3::Cross(Vector3(p2.x, p2.y, 1),
			Vector3(p0.x, p0.y, 1));
	...
	void RasterisationMesh::RasterSpanTriangle(
		...
		if (vertices[0]->y == vertices[1]->y) { // make [0] the highest ,
